...
# Canonicalize the temp dir: on macOS `mktemp -d` returns a path under /var, which
# is a symlink to /private/var, and Nix rejects a chroot store whose path or any
# parent is a symlink. `pwd -P` resolves it to a symlink-free path.
